...
@Component({
  selector: 'app-empresa-notificaciones',
  standalone: true,
    ...
    <div class="page">
      <div class="page-header-row">
        <div>
          ...
          <p class="page-subtitle">{{ noLeidas() > 0 ? noLeidas() + ' sin leer' : 'Todo al día' }}</p>
        </div>
        @if (noLeidas() > 0) {
      ...
      @if (loading()) {
        ...
        <app-empty-state icon="notifications_none" title="Sin notificaciones" description="No tienes notificaciones por el momento." />
      } @else {
        <div class="notifs-list">
          @for (n of notifs(); track n.id) {
            <div class="notif-item" [class.unread]="!n.leida" (click)="marcarLeida(n)">
              <div class="notif-icon" [style.background]="config(n.tipo).color + '15'" [style.color]="config(n.tipo).color">
                <mat-icon>{{ config(n.tipo).icon }}</mat-icon>
              </div>
...
export class EmpresaNotificacionesComponent implements OnInit {
  ...
  config(t: TipoNotificacion) { return TIPO_CONFIG[t] ?? { icon: 'notifications', color: '#6B6B6B' }; }
  ...
  marcarLeida(n: Notificacion) {
    ...
    this.notifSvc.marcarLeida(n.id).subscribe({
      next: () => {
        this.notifs.update(l => l.map(x => x.id === n.id ? { ...x, leida: true } : x));
        this.noLeidas.update(c => Math.max(0, c - 1));
        this.notifSvc.setUnreadCount(Math.max(0, this.noLeidas()));
      },
    });
  }
  ...
  marcarTodas() {
    this.notifSvc.marcarTodasLeidas().subscribe({
      next: () => { this.notifs.update(l => l.map(x => ({ ...x, leida: true }))); this.noLeidas.set(0); this.notifSvc.setUnreadCount(0); },
    });
  }

  eliminar(id: string) {
    this.notifSvc.delete(id).subscribe({ next: () => this.notifs.update(l => l.filter(x => x.id !== id)) });
  }
}
